...
const warnTxt = '/!\\ Be careful, commands will not affect your computer but can mess with your windows93 desktop and saved files...';
...
let diacriticChange = defaultDiacriticsRemovalMap;
...
window.addons = {
	register: function(evt, cb) {
		if (this.callbacks[evt]) this.callbacks[evt].push(cb);
	},
...
function chatKing() {
    ...
    const trollbox_infos = $( "#trollbox_infos_div" )
    trollbox_infos.contextmenu(function() {
        sendMsg('/block '+$(this).children('span:last-child').text())
        return false
    });
    trollbox_infos.click(function() {
...
socket.on('_connected', function (data) {
    const welcomed = localStorage.getItem('.config/trollbox/tbjb_welcomed')
    if (welcomed === 'false') {
...
socket.on('user change nick', function (data) {
    ...
    for (let i = 0; i < blocked.length; i++) {
        if (data[1].home === blocked[i]) return;
    }
    if (data[1].nick) {
...
$el(trollbox_scroll)
    .on('click', 'button', function () {
        if (window.top.$exe) {
            const res = window.top.$exe(this.getAttribute('data-exe'))
...
function getScrollPos() {
    let startDistance = 0;
    const scroll = $('#trollbox_scroll');
    const scrollTop = scroll.scrollTop()+scroll.height();
    let documentHeight = document.getElementById("trollbox_scroll").scrollHeight;
    return parseInt((scrollTop / documentHeight) * 100);
}

function removeDiacritics(str) {
    diacriticChange = diacriticChange || defaultDiacriticsRemovalMap;
    for (let i=0; i<diacriticChange.length; i++) {
...
function flipText(txt) {
    ...
    let convTxt = '';
    for (let c = txt.length - 1; c >= 0; c--) {
        if (flipTable[txt.charAt(c)] !== undefined) {
            convTxt += flipTable[txt.charAt(c)];
        } else {
            convTxt += txt.charAt(c);
        }
    }
    return convTxt;
}
